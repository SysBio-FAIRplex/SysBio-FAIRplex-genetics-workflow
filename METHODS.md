...
| Callset | Consortium | Cohorts | Samples | Build | Source format |
|---|---|---|---|---|---|
...
The §5 QC computes HWE pooled across callsets, so a deviation confined to `wgs_harm` (~1,540 of
~9,800 EUR samples) is averaged against a clean majority before the test sees it. This channel
...
| Stratum | *n* | max η² unfiltered | max η² filtered | Reduction |
|---|---|---|---|---|
...
**In EUR, 0.058% of variants carried essentially all callset structure in the principal
components.** The flagged variants are ~7× enriched for genotype discordance between duplicate
sample pairs, where the true genotype is identical by construction — independent confirmation that
they are technical.
...
**44 contrasts ran; 17 met the ≥100-per-arm viability threshold** (EUR 14, AJ 3). Genomic inflation
was well controlled throughout: **λ_GC 1.0175–1.0549**, the maximum in `PD_vs_AD`, the primary and
most confounded contrast. No contrast required inflation correction.
...
all, and the differential-missingness filter removed 353,068 variants from that contrast — roughly
six times any genuinely cross-program contrast (`AD_vs_DLB` 65,245; `AD_vs_control` 63,393;
`control_amppd_vs_control_ampad` 62,344). Roughly
55 EUR genomes generated more technical asymmetry than the entire AMP-AD/AMP-PD split.
